...
def mis(model, device, test_loader, nimage = 64):
    ...
    dirName = gdrivepath + 'content/mis_class/'
    subdir =  gdrivepath + 'content/mis_class/images'
    ...
    for index in range(0, nimage):
        ...
        path = gdrivepath + 'content/mis_class/images/mis_' + str(index) + '.png'
...
def gen_grad(model, nimage=64, map_overlay= 0.3, img_overlay= 0.7):
  ...
  dirName  = gdrivepath + 'content/map/'
  dir2Name = gdrivepath + 'content/heatmap/'
  ...
  dataset = ImageFolder(root= gdrivepath + 'content/mis_class/', transform=transform)
  ...
  for index, batch in enumerate(dataloader):
        ...
        cvpath = gdrivepath+ 'content/mis_class/images/mis_' + str(index) + '.png'
        img = cv2.imread(cvpath)
        heatmap = cv2.resize(heatmap, (img.shape[1], img.shape[0]))
        heatmap = np.uint8(255 * heatmap)
        heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
        heatmap_path = gdrivepath+ 'content/heatmap/heatmap_' + str(index) + '.png'
        cv2.imwrite(heatmap_path, heatmap.squeeze()) 
        superimposed_img = heatmap * map_overlay + img * img_overlay
        map_path = gdrivepath+ 'content/map/map_' + str(index) + '.png'
...
def plt_grad():
    ...
    for index in range(0, 36):
        ...
        plt.xticks([])
        plt.yticks([])
        path = gdrivepath+ 'content/map/map_' + str(index) + '.png'
        xa = plt.imread(path)
        plt.imshow(xa.squeeze(), cmap='gray_r')
        plt.setp(plt.title(f'Predicted: {pred_list[index]}'), color= 'red')
        plt.setp(plt.xlabel(f'Actual: {true_list[index]}'), color= 'blue')
        plt.tight_layout()
...
def heat_map():
    ...
    for index in range(0, 36):
        ...
        plt.xticks([])
        plt.yticks([])
        path = gdrivepath+'content/heatmap/heatmap_' + str(index) + '.png'
        xa = plt.imread(path)
        plt.imshow(xa.squeeze(), cmap='gray_r')
        plt.setp(plt.title(f'Predicted: {pred_list[index]}'), color= 'red')
        plt.setp(plt.xlabel(f'Actual: {true_list[index]}'), color= 'blue')
